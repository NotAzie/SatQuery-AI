...
async def kg_query(
    ...
    logger.debug("Extracted %d low-level and %d high-level keywords", len(ll_keywords), len(hl_keywords))
...
async def extract_keywords_only(
    ...
    logger.debug("Keyword extraction result parsed successfully")
...
async def _get_node_data(
    ...
    overall_top_k = max(0, query_param.top_k - sum(allocated_top_k))
    ...
    for i, keyword in enumerate(keywords):
        ...
        # ✅ 如果已达到总 top_k，终止
        if len(results) >= query_param.top_k:
            logger.debug("Retrieved %d entity candidates", len(results))
            break
    ...
    if not len(results):
        return "", "", ""
    logger.debug("Merged entity candidates: %d", len(results))
...
async def _get_edge_data(
    ...
    for i, keyword in enumerate(keywords):
        ...
        # ✅ 如果已达到总 top_k，终止
        if len(results) >= query_param.top_k:
            logger.debug("Retrieved %d relationship candidates", len(results))
            break
    ...
    if not len(results):
        return "", "", ""
    logger.debug("Merged relationship candidates: %d", len(results))
